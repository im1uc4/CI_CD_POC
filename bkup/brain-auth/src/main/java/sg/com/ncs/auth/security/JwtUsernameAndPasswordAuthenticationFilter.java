...
public class JwtUsernameAndPasswordAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		List<String> grantedAuthorities = auth.getAuthorities().stream().map(GrantedAuthority::getAuthority)
				.collect(Collectors.toList());

		String username = grantedAuthorities.stream().filter(authority -> authority.contains("username:"))
				.collect(Collectors.toList()).get(0);
		String email = grantedAuthorities.stream().filter(authority -> authority.contains("email:"))
				.collect(Collectors.toList()).get(0);
		String fullname = grantedAuthorities.stream().filter(authority -> authority.contains("fullname:"))
				.collect(Collectors.toList()).get(0);

		claims.put("username", username.split(":")[1]);
		claims.put("email", email.split(":")[1]);
		claims.put("fullname", fullname.split(":")[1]);
		claims.put("sub", auth.getName());

		grantedAuthorities = grantedAuthorities.stream()
				.filter(authority -> !authority.contains("fullname:") && !authority.contains("email:")
						|| authority.contains("username:"))
				.collect(Collectors.toList());

		claims.put("authorities", grantedAuthorities);

		Long now = System.currentTimeMillis();
		String token = Jwts.builder().setClaims(claims).setIssuedAt(new Date(now))
